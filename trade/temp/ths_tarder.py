...
class ThsTarder:
    ...
    def __init__(self, exe_path, account, password, tesseract_cmd):
        ...
        try:
            ...
            # self.app.window(title='网上股票交易系统5.0').exists(timeout=10)
    ...
    def start_exe(self, exe_path, account, password):
        ...
        for open_count in range(10):
            ...
            # 如果以上操作失败，显示失败原因和尝试次数，执行下个循环
                ...
                logger.info('异常步骤：打开下单程序，请检查程序路径和文件完整性', ex)
                logger.info('第[%s]次打开应用失败，3秒后将再尝试' % is_bing)
                time.sleep(3)
    ...
    def login_exe(self, account, password):
        ...
        for password_count in range(4):
            ...
            logger.info('第[%s]次登入中' % login_count)
            # 连接登入窗口
            ...
            if self.app.window(title='网上股票交易系统5.0').exists(timeout=10):
                logger.info('登入成功')
                open_main_window = self.app.window(title=u"网上股票交易系统5.0", control_type="Window")
                # 定位第一个子控件
                ...
                if daxin_dit['class_name'] == '#32770':
                    daxin.close()
                    logger.info('检测到有打新页面，关闭中')
                else:
                    logger.info('没有弹出打新页面，无需关闭')
                self.main_window = self.app.window(title=u"网上股票交易系统5.0", control_type="Window")
                break
            # 如果没有连接到交易主窗口
            else:
                ...
                # 第二种判断方法
                # if self.app.top_window().set_focus().texts() == '网上股票交易系统5.0':
                #     logger.info('登入成功')
                #     break
                # else:
                #     # logger.info(self.login_windows.children_texts())
    ...
    def buy(self, buy_stock_number, buy_price, buy_amount):
        ...
        # 定位价格窗口，其实可以不要，因为同花顺自己会输入当前价格,会导致数据输入错误
        ...
        time.sleep(1)
        # 定位买入控件
        buy_key = temporary_main_win.child_window(title="买入", auto_id="1006", control_type="Button")
    ...
    def sell(self, sell_stock_number, sell_price, sell_amount):
        ...
        time.sleep(1)
        # 定位卖出控件
        sell_key = temporary_main_win.child_window(title="卖出", auto_id="1006", control_type="Button")
    ...
    def selet_entrust_today(self):
        ...
        # self.deal_today_ctrl=self.selet_ctrl.children()[1]
        # 定位当日成交控件
        deal_today_ctrl = selet_ctrl.child_window(title="当日委托", control_type="TreeItem")
        deal_today_ctrl.click_input()
        deal_today_ctrl.draw_outline(colour='red')
        # 调用右侧菜单，获得右侧界面控件
        deal_today_win = self.right_data_win()
...
# 实例化ThsTarder
a = ThsTarder(xiadan_exe_path, stock_account, trader_password, tesseract_cmd)
# 执行买入操作
