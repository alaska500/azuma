...
def back(date_str):
    ...
    for index, row in kzz_spot_top.iterrows():
        ...
        income = calc_income(kzz_min_df, row["债券名称"], symbol, float(row["昨日收盘"]), row["今开"])
        res = res + income
    logger.info("总收益为：" + str(res))
    result_df.to_csv(f"../../res/res_{date_str}.csv")

def calc_income(kzz_min_df, name, symbol, settlement, open):
    buy_price = -1
    buy_index = -1
    change = -99
...
if __name__ == '__main__':
    back("2023-05-10")
    back(datetime.now().strftime("%Y-%m-%d"))
